...
function storeForecast(request, city, func){
    var request_url_1 = `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${initials.appid}` //`/openweather/weather` 
    ...
    req1.then(d => d.json()).then(d => {
        ...
        var request_url_2 = `https://api.openweathermap.org/data/2.5/onecall?lat=${data.lat}&lon=${data.lon}&exclude=minutely&units=metric&appid=${initials.appid}` //`/openweather/data`
            ...
            case location.pathname.startsWith('/index.html') || location.pathname === '/':
                indexPage(city => {
                    storeForecast(request, city, (error, forecastId) => {
                        if(error){
                            return alert(FAILURE_ALERT, `Error occured while searching. Try again`)
                        }
